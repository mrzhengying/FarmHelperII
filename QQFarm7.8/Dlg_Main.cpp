...
BOOL						m_bFishsOpen = FALSE;	//是否已开通鱼塘
int							nYellowLevel = 0;	//黄钻等级
int							nYellowStatus = 0;	//黄钻状态
...
UINT H_GetFishInfo(CString &strHtml,ST_USERINFO &pUserInfo); //处理我的鱼塘数据
UINT H_GetCropsInfo(int nId,ST_CROPSGROW &pCropGrow);	//从INI文件读取指定植物数据
DWORD H_GetFarmLandRipe(ST_CROPLANDINFO pCropLand[24],ST_FISHLANDINFO pFishLand[6]);	//处理农场田地数据,返回最近一次收成的时间
...
UINT G_GetFishInfo(int nId,ST_FISHINFO &pFishInfo);
UINT G_WriteFishInfo(ST_FISHINFO &pFishInfo);
...
void CDlg_Main::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX,IDC_LIST_FISHLANDS,m_List_FishLands);
...
BOOL CDlg_Main::OnInitDialog()	//初始化窗口
{
	...
	//给池塘列表框添加标头
	m_List_FishLands.InsertColumn(0,"编号",NULL,40,-1);
	m_List_FishLands.InsertColumn(1,"名称",NULL,100,-1);
	//m_List_FishLands.InsertColumn(2,"阶段",NULL,80,-1);
	m_List_FishLands.InsertColumn(2,"距成熟",NULL,112,-1);
	//设置列表框整行选择和显示网格
	m_List_FishLands.SetExtendedStyle(LVS_EX_GRIDLINES|LVS_EX_FULLROWSELECT);
	...
	strFishsIni =		strSelfPath + "\\QQFarm_Fishs.ini";

	strLogPath =		strSelfPath + "\\QQFarm_PostRetStr.log";
	...
	{
		//强制更新农作物数据
		WritePrivateProfileString("xmldata","config_data","",strQQFarm_ini);
		nDataVersion ++;
	}
	if (nDataVersion == 3)
	{
	...
	//写入当前版本数据
	WritePrivateProfileString("version","dataversion","5",strQQFarm_ini);
...
M1:
	...
	//获取鱼塘数据
	strPostData.Format("uIdx=%d&uinY=%s&ownerId=%d",pUserInfo.Id,GetQQInfo(GQI_NUM),pUserInfo.Id);
	CString strFishs = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_fish_index",strPostData);
	//处理返回的数据
	H_GetFishInfo(strFishs,pUserInfo);

	//是否开通鱼塘
	if (FindMidStr(strFishs,0,"\"open\":","}",strBuf) != -1)
		m_bFishsOpen = atoi(strBuf);

	if (!m_bFishsOpen)
	{
		//开通鱼塘功能
		strPostData.Format("uinY=%s&yellow=%d&uIdx=%d",GetQQInfo(GQI_NUM),nYellowStatus,pUserInfo.Id);
		strFishs = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_register",strPostData);
		if (FindMidStr(strFishs,0,"\"code\":","}",strBuf) != -1)
		{
			m_bFishsOpen = atoi(strBuf);
			CC_Addtolist("[QQ农场]:开通鱼塘饲养功能成功",COLOR_INFO);
		}
	}
	...
	m_SelfInfo.RipeTime = H_GetFarmLandRipe(pUserInfo.CropLand,pUserInfo.FishLand);
	...
	if (strConfig_data != strConfig_data_ini)
	{
		...
		xml.FindElem("waterPool");
		CString strFishs=xml.GetData();
		...
		while(nOffset!=-1)
		{
			...
			FindMidStr(strBuf,0,"\"isRed\":",",",strItem);
			st_Crops.IsRed=atoi(strItem);
			...
			st_Crops.IsVip=FindMidStr(strBuf,0,"\"isVIP\":",",",strItem)==-1 ? 0 : 1 ;
		...
		CC_Addtolist("[QQ农场]:开始分析鱼类数据",COLOR_INFO);

		Sleep(1000);

		//获取商店鱼类数据
		strPostData.Format("uIdx=%d&uinY=%s",pUserInfo.Id,GetQQInfo(GQI_NUM));
		strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_list",strPostData);

		//开始分析鱼的数据
		nOffset = 0;

		nOffset=FindMidStr(strFishs,nOffset,"\"crop_name\": \"","}",strBuf);
		while(nOffset!=-1)
		{
			ST_FISHINFO st_FishInfo;
			CString strItem;

			FindMidStr(strBuf,0,"\"id\":",",",strItem);
			st_FishInfo.Id=atoi(strItem);
			G_GetFishInfo(st_FishInfo.Id,st_FishInfo);

			FindMidStr(strBuf,0,"","\"",strItem);
			strcpy(st_FishInfo.Name,strItem.GetBuffer(strItem.GetLength()));
			strItem.ReleaseBuffer();

			CString strTxt;
			FindMidStr(strBuf,0,"\"cycle\": \"","\", \"",strTxt);
			nIndex = 0;
			nIndex = FindMidStr(strTxt,nIndex,"",",",strItem);
			st_FishInfo.Cycle[0] = atol(strItem);
			nIndex = FindMidStr(strTxt,nIndex,"",",",strItem);
			st_FishInfo.Cycle[1] = atol(strItem);
			nIndex = FindMidStr(strTxt,nIndex,"","\"",strItem);
			st_FishInfo.Cycle[2] = atol(strItem);

			FindMidStr(strBuf,0,"\"exp\":",",",strItem);
			st_FishInfo.Exp=atoi(strItem);
			FindMidStr(strBuf,0,"\"isMill\":",",",strItem);
			st_FishInfo.IsMill=atoi(strItem);
			FindMidStr(strBuf,0,"\"isRestaurant\":",",",strItem);
			st_FishInfo.IsRestaurant=atoi(strItem);

			FindMidStr(strBuf,0,"\"lock_crystal\": \"",", \"",strTxt);
			nIndex = 0;
			nIndex = FindMidStr(strTxt,nIndex,"",",",strItem);
			st_FishInfo.Lock_Crystal=atoi(strItem);
			nIndex = FindMidStr(strTxt,nIndex,"","\"",strItem);
			st_FishInfo.Lock_Num=atoi(strItem);

			FindMidStr(strBuf,0,"\"lock_money\": \"","\",",strItem);
			st_FishInfo.Lock_Money=atoi(strItem);
			FindMidStr(strBuf,0,"\"mature\":",",",strItem);
			st_FishInfo.Mature=atoi(strItem);
			FindMidStr(strBuf,0,"\"output\":",",",strItem);
			st_FishInfo.Output=atoi(strItem);
			FindMidStr(strBuf,0,"\"pool_size\":",",",strItem);
			st_FishInfo.Pool_Size=atoi(strItem);
			FindMidStr(strBuf,0,"\"price\":",",",strItem);
			st_FishInfo.Price=atoi(strItem);
			FindMidStr(strBuf,0,"\"sale\":",",",strItem);
			st_FishInfo.Sale=atoi(strItem);
			FindMidStr(strBuf,0,"\"show\":",",",strItem);
			st_FishInfo.Show=atoi(strItem);

			CString strId,strBuf2;
			strId.Format("\"fid\":%d,",st_FishInfo.Id);
			if (FindMidStr(strHtml,0,strId,"}",strBuf2) != -1)
			{
				FindMidStr(strBuf2,0,"\"lock\":",",",strItem);
				st_FishInfo.Lock = atoi(strItem);
				FindMidStr(strBuf2,0,"\"type\":","}",strItem);
				st_FishInfo.Type = atoi(strItem);
			}

			if (nOffset == -1)
				break;
			G_WriteFishInfo(st_FishInfo);
			nOffset=FindMidStr(strFishs,nOffset,"\"crop_name\": \"","}",strBuf);
		}
	...
	//临时存储未解锁鱼的数据
	std::vector<ST_FISHINFO> v_Fishs;
	char szFishs[SHORTUSERLEN]={0};
	char szFish[SHORTLEN]={0};
	//取所有Section项,单个字符串以0结尾,整个字符串以00结尾
	GetPrivateProfileSectionNames(szFishs,SHORTUSERLEN,strFishsIni);
	int nPos = 0;
	while(szFishs[nPos] != 0)
	{
		strcpy(szFish,szFishs+nPos);	//得到单个的Section
		ST_FISHINFO pFishInfo;
		G_GetFishInfo(atoi(szFish),pFishInfo);	//读取这个Section的数据
		if (pFishInfo.Show == TRUE && pFishInfo.Lock == 2)
			v_Fishs.push_back(pFishInfo);
		nPos += strlen(szFish) + 1;
	}
	if (v_Fishs.size() > 0)
	{
		F_GetUserCrystal(NULL);	//获取仓库水晶
		Sleep(1000);
		for (int i=0;i<v_Fishs.size();i++)
		{
			BOOL bHaveCrystal = FALSE;
			BOOL bHaveMoney = FALSE;
			ST_CRYSTALINFO pCrystalInfo;

			if (v_Fishs[i].Lock_Crystal == 0)	//如果为零,则是不需要水晶
			{
				bHaveCrystal = TRUE;
			}
			else
			{
				G_GetCrystalInfo(v_Fishs[i].Lock_Crystal,pCrystalInfo);
				if (pCrystalInfo.nNum >= v_Fishs[i].Lock_Num)	//仓库数量需大于需求数量
					bHaveCrystal = TRUE;
			}

			if (pUserInfo.Money > v_Fishs[i].Lock_Money)	//金币数量
				bHaveMoney = TRUE;

			if (bHaveCrystal && bHaveMoney)		//如果符合条件则进行解锁
			{
				strPostData.Format("uIdx=%d&fid=%d&uinY=%s",m_SelfInfo.Id,v_Fishs[i].Id,GetQQInfo(GQI_NUM));
				CString strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_unlock",strPostData);
				...
				if (nCode == TRUE)
				{
					pCrystalInfo.nNum -= v_Fishs[i].Lock_Num;
					G_WriteCrystalInfo(pCrystalInfo);
					v_Fishs[i].Lock = 1;
					G_WriteFishInfo(v_Fishs[i]);
					pUserInfo.Money -= v_Fishs[i].Lock_Money;

					CString strPrintf;
					strPrintf.Format("[QQ农场]:解锁了一种鱼苗[%s],花费了%d个金币",v_Fishs[i].Name,v_Fishs[i].Lock_Money);
		...
		while(TRUE)
		{
			...
			if (bScanFriends)
			{
				...
				for (int i=0;i<v_FriendsInfo.size();i++)	//循环读取简易好友列表数组 
				{
					...
					v_FriendsInfo[i].RipeTime = H_GetFarmLandRipe(pUserInfo.CropLand,pUserInfo.FishLand); //获取农场最近成熟的时间,没有则返回0
			...
			for (int i=0;i<v_FriendsInfo.size();i++)
			{
					...
					m_Config.bFdSc)
				{
					...
					if (m_Config.bFdSc || m_Config.bFdWcs)
					{
						...
						ST_USERINFO pUserInfo;	//从INI文件中读出获取的数据
						G_GetUserInfo(v_FriendsInfo[i].Id,pUserInfo);
						v_FriendsInfo[i].RipeTime = H_GetFarmLandRipe(pUserInfo.CropLand,pUserInfo.FishLand);
						v_FriendsInfo[i].CheckTime = dwServerTime;				//记录一下处理的时间
						G_WriteUserList(v_FriendsInfo[i]);						//写入INI文件
						S_ShowFriendsRipeTimeOnPos(i);							//显示一下距成熟还有多久,没有则空
						Sleep(3000);
			...
			if (bGetStatusFilter)
			{
				...
				if (m_Config.bFdWcs && bWcsFriends)
				{
...
DWORD H_GetFarmLandRipe(ST_CROPLANDINFO pCropLand[24],ST_FISHLANDINFO pFishLand[6])
{
	...
	for (int i=0;i<24;i++)
	{
		if (pCropLand[i].a > 0 && pCropLand[i].n == TRUE && pCropLand[i].b < 6)	//如果不是空地,而且没有偷过,则进入
		{
			...
			if (pCropLand[i].IsRed == 2 && pCropGrow.IsRed != 2)	//如果是黑土地,减少20%的时间
			{
				dwCropCrow = pCropGrow.CropGrow[4] * 0.8;
				DWORD dwTmp1 = dwCropCrow % 600;
				if (dwCropCrow > dwTmp1)
					dwCropCrow = dwCropCrow - dwTmp1;
			}
			else
				dwCropCrow = pCropGrow.CropGrow[4];
			
			dwCropTime = pCropLand[i].q + dwCropCrow;

			if (dwCropTime > (dwServerTime - 14400))
				v_ripe.push_back(dwCropTime);
		}
	}
	for (i=0;i<6;i++)
	{
		if (pFishLand[i].fId > 0 && pFishLand[i].o == 0 && pFishLand[i].l == 0 && pFishLand[i].s == TRUE)
		{
			ST_FISHINFO pFishInfo;
			G_GetFishInfo(pFishLand[i].fId,pFishInfo);
			DWORD dwFishTime = 0;
			dwFishTime = pFishLand[i].p + (pFishInfo.Mature * 60 * 60);
			if (dwFishTime > (dwServerTime - 14400))
				v_ripe.push_back(dwFishTime);				
		}
	}
	if (v_ripe.size() > 0)
	{
...
UINT H_GetFarmInfo(CString &strHtml,ST_USERINFO &pUserInfo)
{
	...
	memcpy(&pUserInfo.CropLand,&pClrUserInfo.CropLand,sizeof(pClrUserInfo.CropLand));
	...
	CString strBuf;	//临时文本
		...
		{
			//清空一下数据先
			ST_MYBEASTINFO pClrMyBeastInfo[3];
			memcpy(m_MyBeast,pClrMyBeastInfo,sizeof(ST_MYBEASTINFO)*3);
...
UINT H_GetFishInfo(CString &strHtml,ST_USERINFO &pUserInfo)
{
	//清空一下数据先
	ST_USERINFO pClrUserInfo;
	memcpy(&pUserInfo.FishLand,&pClrUserInfo.FishLand,sizeof(pClrUserInfo.FishLand));
	
	int nOffset = 0,nCount = 0;
	nOffset = strHtml.Find("\"fish\":",0);
	if (nOffset != -1)
	{
		CString strFish,strBuf;
		nOffset=FindMidStr(strHtml,nOffset,"{","}",strFish);
		while(nOffset != -1)
		{
			int nCount = 0;
			//位置
			if (FindMidStr(strFish,0,"\"i\":",",",strBuf) != -1)
			{
				nCount = atoi(strBuf);
				pUserInfo.FishLand[nCount].i = nCount;
			}
			//鱼的ID
			if (FindMidStr(strFish,0,"\"fid\":",",",strBuf) != -1)
				pUserInfo.FishLand[nCount].fId = atoi(strBuf);
			//剩余产量
			if (FindMidStr(strFish,0,"\"l\":",",",strBuf) != -1)
				pUserInfo.FishLand[nCount].l = atoi(strBuf);
			//总产量
			if (FindMidStr(strFish,0,"\"o\":",",",strBuf) != -1)
				pUserInfo.FishLand[nCount].o = atoi(strBuf);
			//饲养时间
			if (FindMidStr(strFish,0,"\"p\":",",",strBuf) != -1)
				pUserInfo.FishLand[nCount].p = atol(strBuf);
			//是否可偷取
			if (FindMidStr(strFish,0,"\"s\":[","]",strBuf) != -1)
			{
				CString strId;
				strId.Format("%d",m_SelfInfo.Id);
				pUserInfo.FishLand[nCount].s = strBuf.Find(strId,0) == -1 ? 1:0;
			}
...
//返回购买哪种种子的ID
UINT H_GetBuySeedId(int bIsRed)
{
	//最终返回的种子ID
	UINT nSeedId = 0;
	...
	while(szCrops[nPos] != 0)
	{
		...
		if (pCropGrow.nSeedNum > 0 && bIsRed >= pCropGrow.IsRed)
...
UINT F_FarmUpgrade(ST_USERINFO &pUserInfo)
{
	...
	FindMidStr(strHtml,0,"\"code\":",",",strBuf);	//code为1则成功		   
	nCode = atoi(strBuf);
	FindMidStr(strHtml,0,"需要等级","级",strBuf);
	nLevel = atoi(strBuf);
	FindMidStr(strHtml,0,"金币","，",strBuf);
	nMoney = atoi(strBuf);
	//if (nCode == TRUE)
	{
		if (nLevel > 0 && nMoney > 0 && m_SelfInfo.Lvl >= nLevel && pUserInfo.Money > nMoney)
...
//升级为黑土地
UINT F_FarmUpgradeBlack2(ST_USERINFO &pUserInfo)
{
	UINT nResult = FALSE;

	CString strPostData;
	strPostData.Format("uIdx=%d&op=0&uinY=%s",m_SelfInfo.Id,GetQQInfo(GQI_NUM));
	CString strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_farm_upgrade_black",strPostData);
	...
	if (nCode == TRUE)
	{
		FindMidStr(strHtml,0,"\"cd\":",",",strBuf);
		nCd = atol(strBuf);
		if (dwServerTime > nCd)
		{
			FindMidStr(strHtml,0,"\"level\":",",",strBuf);
			...
			if (nLevel > 0 && nMoney > 0 && m_SelfInfo.Lvl >= nLevel && pUserInfo.Money > nMoney)
			{
				strPostData.Format("uIdx=%d&op=1&uinY=%s",m_SelfInfo.Id,GetQQInfo(GQI_NUM));
				strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_farm_upgrade_black",strPostData);
				...
				if (nCode == TRUE)
				{
					FindMidStr(strHtml,0,"\"money\":",",",strBuf);
					nMoney = atoi(strBuf);
					FindMidStr(strHtml,0,"\"place\":",",",strBuf);
					nPlace = atoi(strBuf);

					pUserInfo.CropLand[nPlace].IsRed = 2;

					CString strPrintf;
					strPrintf.Format("[QQ农场]:升级了一块土地为黑土地,花费了%d个金币",nMoney);
					Addtolist(strPrintf,COLOR_INFO);
					G_WriteUserInfo(pUserInfo);
...
//收获已成熟的鱼
UINT F_FarmHarvestFish(ST_USERINFO &pUserInfo)
{
	UINT nResult = FALSE;
	for (int i=0;i<6;i++)
	{
		if (pUserInfo.FishLand[i].fId > 0 &&
			pUserInfo.FishLand[i].o	> 0 &&
			pUserInfo.FishLand[i].l > 0)
		{
			CString strPostData;
			strPostData.Format("uIdx=%d&uinY=%s&index=%d",m_SelfInfo.Id,GetQQInfo(GQI_NUM),pUserInfo.FishLand[i].i);
			CString strHtml = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_fish_harvest",strPostData);
			...
			if (nCode == TRUE)
			{
				FindMidStr(strHtml,0,"\"exp\":",",",strBuf);
				nExp = atoi(strBuf);
				FindMidStr(strHtml,0,"\"o\":","}",strBuf);
				nNum = atoi(strBuf);

				if (pUserInfo.FishLand[i].fId != pFishInfo.Id)
					G_GetFishInfo(pUserInfo.FishLand[i].fId,pFishInfo);
				CString strPrintf;
				strPrintf.Format("[QQ农场]:收获了自己池塘里%d号位置的%d个%s,增加了%d点经验",pUserInfo.FishLand[i].i+1,nNum,pFishInfo.Name,nExp);
				Addtolist(strPrintf,COLOR_INFO);

				pUserInfo.FishLand[i].fId = 0;
				pUserInfo.FishLand[i].f = 0;
				pUserInfo.FishLand[i].i = -1;
				pUserInfo.FishLand[i].l = 0;
				pUserInfo.FishLand[i].o = 0;
				pUserInfo.FishLand[i].p = 0;
				pUserInfo.FishLand[i].s = FALSE;
...
//购买鱼苗
UINT F_FarmBuyFish(int nId, int nNum)
{
	CString strPostData;
	strPostData.Format("fid=%d&num=%d&uIdx=%d&uinY=%s",nId,nNum,m_SelfInfo.Id,GetQQInfo(GQI_NUM));
	CString strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_buy",strPostData);
	CString strBuf;
	...
	FindMidStr(strHtml,0,"\"money\":",",",strBuf);
	nMoney = atoi(strBuf);
	strName = CFindMidStr(strHtml,"\"name\":\"","\",");
	...
	if (nCode == TRUE)
	{
		...
		CString strPrintf;
		strPrintf.Format("[QQ农场]:购买了%d个%s的鱼苗,花费了%d个金币",nNum,strName,nMoney);
		if (strDirection != "")
			strPrintf += "\r\n→" + N_HtmlToText(strDirection);
		Addtolist(strPrintf,COLOR_INFO);
		return TRUE;
...
BOOL C_FishsLvlCmp(ST_FISHINFO pFishInfo1,ST_FISHINFO pFishInfo2)
{
	return pFishInfo1.Sale > pFishInfo2.Sale;
}
//返回购买哪种鱼苗的ID
UINT H_GetBuyFishId(int nSize)
{
	//最终返回的鱼苗ID
	UINT nFishId = 0;
	std::vector<ST_FISHINFO> v_Fishs;	//可购买的鱼苗
	std::vector<ST_FISHINFO> v_HaveFishs;  //背包里有的鱼苗
	char szFishs[SHORTUSERLEN]={0};
	char szFish[SHORTLEN]={0};
	//取所有Section项,单个字符串以0结尾,整个字符串以00结尾
	GetPrivateProfileSectionNames(szFishs,SHORTUSERLEN,strFishsIni);
	...
	while(szFishs[nPos] != 0)
	{
		...
		ST_FISHINFO pFishInfo;
		G_GetFishInfo(atoi(szFish),pFishInfo);	//读取这个Section的数据
		if (pFishInfo.Show == TRUE && pFishInfo.Lock == TRUE)
			v_Fishs.push_back(pFishInfo);
		if (pFishInfo.nSeedNum > 0)
			v_HaveFishs.push_back(pFishInfo);
		nPos += strlen(szFish) + 1;
	}
	std::sort(v_Fishs.begin(),v_Fishs.end(),C_FishsLvlCmp);	//根据种子等级排序
	
	for (int i=0;i<v_Fishs.size();i++)
	{
		if (v_Fishs[i].Pool_Size <= nSize)
		{
			nFishId = v_Fishs[i].Id;
			break;
		}
	}
	
	if (v_HaveFishs.size() > 0)
	{
		for (i=0;i<v_HaveFishs.size();i++)
		{
			if (v_HaveFishs[i].Pool_Size <= nSize)
			{
				nFishId = v_HaveFishs[i].Id;
				break;
			}
		}
	}
	...
	ST_FISHINFO pFishInfo;
	G_GetFishInfo(nFishId,pFishInfo);	
	if (pFishInfo.nSeedNum > 0)
	{
		pFishInfo.nSeedNum--;
		G_WriteFishInfo(pFishInfo);
	}
	else
	{
		if (F_FarmBuyFish(nFishId,1) == FALSE)
			nFishId = 0;
	}
	
	if (!v_Fishs.empty())
		v_Fishs.clear();
	if (!v_HaveFishs.empty())
		v_HaveFishs.clear();
	return nFishId;
}
//饲养鱼苗
UINT F_FarmPlantFish(ST_USERINFO &pUserInfo)
{
	UINT nResult = FALSE;
	...
	for (int i=0;i<6;i++)
	{
		if (pUserInfo.FishLand[i].fId > 0)
		{
			if (pUserInfo.FishLand[i].fId != pFishInfo.Id)
				G_GetFishInfo(pUserInfo.FishLand[i].fId,pFishInfo);
			nSize -= pFishInfo.Pool_Size;
		}
	}

	if (m_Config.bSelfBp == FALSE || m_Config.nPlantingType != 0)
	{
		F_GetUserSeed(NULL);
		Sleep(1000);
	}
	...
	while(nSize > 0)
	{
		UINT nFishId = H_GetBuyFishId(nSize);
		Sleep(1000);
		if (nFishId != 0)
		{
			if (nFishId != pFishInfo.Id)
				G_GetFishInfo(nFishId,pFishInfo);
			nSize -= pFishInfo.Pool_Size;
			
			CString strPostData;
			strPostData.Format("uinY=%s&uIdx=%d&fid=%d",GetQQInfo(GQI_NUM),m_SelfInfo.Id,nFishId);
			CString strHtml = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_fish_plant",strPostData);
			...
			if (nCode == TRUE)
			{
				FindMidStr(strHtml,0,"\"exp\":",",",strBuf);
				nExp = atoi(strBuf);
				FindMidStr(strHtml,0,"\"i\":",",",strBuf);
				nCount = atoi(strBuf);
				pUserInfo.FishLand[nCount].fId = nFishId;
				FindMidStr(strHtml,0,"\"p\":","}",strBuf);
				pUserInfo.FishLand[nCount].p = atol(strBuf);

				CString strPrintf;
				strPrintf.Format("[QQ农场]:饲养了一条%s的鱼苗,获得了%d点经验",pFishInfo.Name,nExp);
				Addtolist(strPrintf,COLOR_INFO);
				nResult = TRUE;
			}
			Sleep(800);
...
UINT H_HandleMyFarm(LPVOID pParam)
{
	...
	UINT nHarvestFish = FALSE;//是否有收获鱼
	UINT nPlantFish	= FALSE; //是否有饲养鱼
	...
	if (m_Config.bSelfSc)
	{
		...
		if (nHarvest)			//如果有则暂停两秒
		{
			...
			if (m_Config.bUpRed && m_SelfInfo.Lvl > 27)		//升级为红土地
			{
				...
				for (int i=0;i<24;i++)
				{
					if (pUserInfo.CropLand[i].IsRed > 0)
						nRed++;
				}
			...
			if (m_Config.bUpBlack2 && m_SelfInfo.Lvl > 39)		//升级为黑土地
			{
				int nBlack2 = 0;
				for (int i=0;i<24;i++)
				{
					if (pUserInfo.CropLand[i].IsRed > 1)
						nBlack2++;
				}
				int nLvl = m_SelfInfo.Lvl - 39;
				if (nLvl > nBlack2)
				{
					F_FarmUpgradeBlack2(pUserInfo);
					Sleep(2000);
				}
			}
		}
	...
	if (m_bFishsOpen)
	{
		if (m_Config.bSelfScFish)
		{
			nHarvestFish = F_FarmHarvestFish(pUserInfo); //收获池塘里的鱼
			if (nHarvestFish)
			{
				Sleep(2000);
				F_GetUserFish(NULL);	//获取仓库收获
				Sleep(2000);
				F_SaleUserFish(NULL);	//卖出所有收获
				Sleep(2000);
			}
		}
		if (m_Config.bSelfBpFish)
		{
			nPlantFish = F_FarmPlantFish(pUserInfo); //饲养鱼
			if (nPlantFish)
				Sleep(2000);
		}
	}

	if (nHarvest||nScarify||nPlanting||nHarvestFish||nPlantFish)
	{
		if (m_SelfLandId == m_SelfInfo.Id)
			S_ShowMyRipeTime();
		return TRUE;
...
//偷取好友成熟的鱼
UINT F_FarmScroungeFish(ST_USERINFO &pUserInfo)
{
	UINT nResult = FALSE;
	...
	for (int i=0;i<6;i++)
	{
		if (pUserInfo.FishLand[i].fId > 0 &&
			pUserInfo.FishLand[i].o > 0 &&
			pUserInfo.FishLand[i].l > 0 &&
			pUserInfo.FishLand[i].s == TRUE &&
			(pUserInfo.FishLand[i].l > (((pUserInfo.FishLand[i].o*60)/100)+1)))
		{
			CString strPostData;
			strPostData.Format("index=%d&ownerId=%d&uinY=%s&uinX=%d&uIdx=%d",pUserInfo.FishLand[i].i,pUserInfo.Id,GetQQInfo(GQI_NUM),pUserInfo.QQ,m_SelfInfo.Id);
			CString strHtml = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_fish_steal",strPostData);
			...
			if (nCode == TRUE)
			{
				FindMidStr(strHtml,0,"\"harvest\":",",",strBuf);
				nHarvest = atoi(strBuf);
				pUserInfo.FishLand[i].s = FALSE;
				...
				CString strPrintf;
				strPrintf.Format("[QQ农场]:偷取了好友[%s]池塘里%d号位的%d条%s",pUserInfo.Name,pUserInfo.FishLand[i].i+1,nHarvest,pFishInfo.Name);
				Addtolist(strPrintf,COLOR_INFO);
				nResult = TRUE;
			}
			Sleep(800);
...
UINT H_HandleFriendFarm(LPVOID pParam)
{
	...
	if (F_FarmAttackBeast(pUserInfo) == TRUE)	//攻击野生动物
		Sleep(2000);
	if (F_FarmRaiseBeast(pUserInfo) == TRUE)  //放养野生动物
		Sleep(2000);

	if (m_Config.bFdScFish)
	{
		if (F_FarmScroungeFish(pUserInfo) == TRUE) //偷取鱼
			Sleep(2000);
	}
...
UINT F_ScanFarmInfo(LPVOID pParam)
{
	...
	if (pUserList.CheckTime + 14400 < dwServerTime)
	{
		...
		//获取鱼塘数据
		if (nId == m_SelfInfo.Id)
			strPostData.Format("uIdx=%d&uinY=%s&ownerId=%d",nId,GetQQInfo(GQI_NUM),nId);
		else
			strPostData.Format("uinX=%d&uIdx=%d&uinY=%s&ownerId=%d",pUserInfo.QQ,m_SelfInfo.Id,GetQQInfo(GQI_NUM),pUserInfo.Id);

		strHtml = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_fish_index",strPostData);

		H_GetFishInfo(strHtml,pUserInfo);	//处理返回的鱼塘数据
...
UINT F_GetUserSeed(LPVOID pParam)
{
	...
	while(nOffset!=-1)
	{
		...
		ST_FISHINFO pFishInfo;
		//鱼苗
		if (FindMidStr(strBuf,0,"\"fId\":",",",strItem) != -1)
		{
			pFishInfo.Id = atoi(strItem);
			int nType;
			FindMidStr(strBuf,0,"\"type\":","}",strItem);
			nType = atoi(strItem);
			if (nType == 23)
			{
				G_GetFishInfo(pFishInfo.Id,pFishInfo);
				//数量
				FindMidStr(strBuf,0,"\"amount\":",",",strItem);
				pFishInfo.nSeedNum = atoi(strItem);
				G_WriteFishInfo(pFishInfo);
			}
		}
...
UINT F_SaleUserCrop(LPVOID pParam)
{
	...
	if (strCrops != "" && nNum >= m_Config.nSaleNum)
	{
		...
		if (nCode == TRUE)
		{
			...
			G_WriteUserInfo(pUserInfo);
			CString strPrintf;
			strPrintf.Format("[QQ农场]:卖出仓库收获的植物成功(未锁定的),获得了%d个金币",nMoney);
			Addtolist(strPrintf,COLOR_INFO);
			return TRUE;
		}
	}
...
//获取用户仓库收获的鱼
UINT F_GetUserFish(LPVOID pParam)
{
	char szFishs[SHORTUSERLEN]={0};
	char szFish[SHORTLEN]={0};
	//取所有Section项,单个字符串以0结尾,整个字符串以00结尾
	...
	while(szFishs[nPos] != 0)
	{
		...
		ST_FISHINFO pFishInfo;
		G_GetFishInfo(atoi(szFish),pFishInfo);	//读取这个Section的数据
		pFishInfo.nCropNum = 0;				//清空仓库收获数量
		G_WriteFishInfo(pFishInfo);		//重新写入配置文件
		nPos += strlen(szFish) + 1;
	}
	
	CString strPostData;
	strPostData.Format("uIdx=%d&uinY=%s",m_SelfInfo.Id,GetQQInfo(GQI_NUM));
	CString strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_user_rep",strPostData);
	int nOffset = 0;
	CString strBuf;
	nOffset = FindMidStr(strHtml,nOffset,"{","}",strBuf);
	while(nOffset!=-1)
	{
		...
		if (FindMidStr(strBuf,0,"\"fid\":",",",strItem) != -1)
		{
			...
			G_GetFishInfo(pFishInfo.Id,pFishInfo);
			//数量
			FindMidStr(strBuf,0,"\"num\":",",",strItem);
			pFishInfo.nCropNum = atoi(strItem);
			//锁定状态
			if (!pFishInfo.IsLock)
			{
				FindMidStr(strBuf,0,"\"lock\":",",",strItem);
				pFishInfo.IsLock = atoi(strItem);
			}
...
//卖出仓库中收获的鱼
UINT F_SaleUserFish(LPVOID pParam)
{
	//可卖出的数量
	int nNum = 0;
	CString strFishs;
	...
	while(szFishs[nPos] != 0)
	{
		...
		ST_FISHINFO pFishInfo;
		G_GetFishInfo(atoi(szFish),pFishInfo);	//读取这个Section的数据
		if (pFishInfo.nCropNum > 0 && !pFishInfo.IsLock)
		{
			CString strId;
			strId.Format("%d",pFishInfo.Id);
			strFishs = strFishs + strId + "%2C";
			nNum += pFishInfo.nCropNum;
			pFishInfo.nCropNum = 0;
			G_WriteFishInfo(pFishInfo);
		}
		nPos += strlen(szFish) + 1;
	}
	if (strFishs != "" && nNum >= m_Config.nSaleNum)
	{
		strFishs = strFishs.Left(strFishs.GetLength()-3);
		CString strPostData;
		strPostData.Format("fIds=%s&uIdx=%d&uinY=%s",strFishs,m_SelfInfo.Id,GetQQInfo(GQI_NUM));
		CString strHtml = N_FarmPostUrl("farm.qzone.qq.com","/cgi-bin/cgi_fish_sale",strPostData);
		...
		if (nCode == TRUE)
		{
			...
			G_WriteUserInfo(pUserInfo);
			CString strPrintf;
			strPrintf.Format("[QQ农场]:卖出仓库收获的鱼类成功(未锁定的),获得了%d个金币",nMoney);
			Addtolist(strPrintf,COLOR_INFO);
			return TRUE;
		}
	}
...
//获取用户仓库的水晶
UINT F_GetUserCrystal(LPVOID pParam)
{
	char szCrystals[SHORTUSERLEN]={0};
	char szCrystal[SHORTLEN]={0};
	//取所有Section项,单个字符串以0结尾,整个字符串以00结尾
	GetPrivateProfileSectionNames(szCrystals,SHORTUSERLEN,strCrystalIni);
	int nPos = 0;
	while(szCrystals[nPos] != 0)
	{
		strcpy(szCrystal,szCrystals+nPos);	//得到单个的Section
		ST_CRYSTALINFO pCrystalInfo;
		G_GetCrystalInfo(atoi(szCrystal),pCrystalInfo);	//读取这个Section的数据
		pCrystalInfo.nNum = 0;				//清空仓库收获数量
		G_WriteCrystalInfo(pCrystalInfo);		//重新写入配置文件
		nPos += strlen(szCrystal) + 1;
	}
	
	CString strPostData;
	strPostData.Format("uIdx=%d&type=9&uinY=%s",m_SelfInfo.Id,GetQQInfo(GQI_NUM));
	CString strHtml = N_FarmPostUrl("nc.qzone.qq.com","/cgi-bin/cgi_farm_get_usercrystal",strPostData);
	int nOffset = 0;
	if ((nOffset=strHtml.Find("\"info\":",0)) != -1)
	{
		CString strBuf;
		nOffset = FindMidStr(strHtml,nOffset,"{","}",strBuf);
		while(nOffset!=-1)
		{
			...
			if (FindMidStr(strBuf,0,"\"cId\":",",",strItem) != -1)
			{
				pCrystalInfo.Id = atoi(strItem);
				G_GetCrystalInfo(pCrystalInfo.Id,pCrystalInfo);
				//数量
				FindMidStr(strBuf,0,"\"amount\":",",",strItem);
				pCrystalInfo.nNum = atoi(strItem);
				//销售价格
				FindMidStr(strBuf,0,"\"price\":",",",strItem);
				pCrystalInfo.SalePrice = atoi(strItem);
				//写入配置文件
				G_WriteCrystalInfo(pCrystalInfo);
			}
...
void S_ShowMyRipeTime()
{
	...
	for (int i=0;i<24;i++)
	{
		...
		if (nId > 0)
		{
			...
			if (nNum < 6)
			{
				...
				CTimeSpan t3;
				int nD,nH,nM;
				if (pUserInfo.CropLand[i].IsRed == 2 && pCropGrow.IsRed != 2)
				{
					DWORD dwTmp1 = pCropGrow.CropGrow[4] * 0.8;
					DWORD dwTmp2 = dwTmp1 % 600;
					if (dwTmp1 > dwTmp2)
						dwTmp1 = dwTmp1 - dwTmp2;
					t1 = pUserInfo.CropLand[i].q + dwTmp1;
				}
				else
	...
	hDlg_Main->m_List_FishLands.DeleteAllItems();
	ST_FISHINFO pFishInfo;	//鱼类数据
	int nCount = 0;
	for (i=0;i<6;i++)
	{
		int nfId = pUserInfo.FishLand[i].fId; //鱼的ID
		if (nfId > 0)
		{
			if (nfId != pFishInfo.Id)	//如果ID不同,则重新读取数据
				G_GetFishInfo(nfId,pFishInfo);

			char szNum[SHORTLEN]={0};
			itoa(i+1,szNum,10);
			//编号
			nCount = hDlg_Main->m_List_FishLands.InsertItem(nCount,szNum);
			//鱼的名称
			hDlg_Main->m_List_FishLands.SetItemText(nCount,1,pFishInfo.Name);
			//距收获
			CString strBuf;
			CTime t1,t2;
			CTimeSpan t3;
			int nD,nH,nM;
			t1 = pUserInfo.FishLand[i].p + ((pFishInfo.Mature * 60) * 60);
			t2 = dwServerTime;
			if (t2 < t1)
			{
				t3 = t1 - t2;
				...
				if (nD > 0)
					nH += nD * 24;
				strBuf.Format("%d小时%d分",nH,nM);
				hDlg_Main->m_List_FishLands.SetItemText(nCount,2,strBuf);
			}
			else
			{
				hDlg_Main->m_List_FishLands.SetItemText(nCount,2,"可收获");
			}
...
UINT G_GetFishInfo(int nId,ST_FISHINFO &pFishInfo)
{
	char szuId[SHORTLEN]={0};
	itoa(nId,szuId,10);
	return GetPrivateProfileStruct(szuId,"FishInfo",&pFishInfo,sizeof(ST_FISHINFO),strFishsIni);
}
UINT G_WriteFishInfo(ST_FISHINFO &pFishInfo)
{
	char szuId[SHORTLEN]={0};
	itoa(pFishInfo.Id,szuId,10);
	return WritePrivateProfileStruct(szuId,"FishInfo",&pFishInfo,sizeof(ST_FISHINFO),strFishsIni);
}
