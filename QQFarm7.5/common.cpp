...
CString			strQqFarmKey;
CString			strPostHeader;
DWORD			dwServerTime;
CString			strSelfPath;
CString			strQQFarm_ini;
CString			strCropsIni;
CString			strAddonIni;
CString			strBeastIni;
...
CString FarmTimeMD5FarmKey(time_t &time)
{
	time = ::time(0);
	CString strTime,strTemp;
	strTime.Format("%d",time);
	strTemp.Format("%s%s",strTime,strQqFarmKey.Right(strQqFarmKey.GetLength() - time%10));
	
	char chMd5[33];
	CC_GetStrMd5(strTemp.GetBuffer(strTemp.GetLength()),chMd5);
	strTemp.ReleaseBuffer();
	
	return CString(chMd5);
}
